## Hi there! I'm Gurleen Kaur

### ABOUT ME

- **Computer Science Engineering** graduate (2026)
- Interested in **Data Engineering, Software, Analytics, and Data Science**.
- Currently exploring **how data pipeline works?**
- Enjoy working with data end-to-end — from **cleaning** to **optimizing complex SQL queries for performance metrics**.
- Hands-on experience with **Ingesting raw, unorganized datasets and transforming them into analytics-ready models**.
- Strong foundation in **C++, Python, SQL, object-oriented programming, and relational databases**.

---
---

### TECH STACK

**Languages:**  
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=postgresql&logoColor=white)

**Databases & Storage:**  
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)

**Data Processing & Analytics:**  
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white)

**Visualization & Web Apps:**  
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=flat)
![Seaborn](https://img.shields.io/badge/Seaborn-5A5A5A?style=flat)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=flat&logo=streamlit&logoColor=white)

**Tools & Development:**  
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=flat&logo=visual-studio-code&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat&logo=jupyter&logoColor=white)
![Excel](https://img.shields.io/badge/Excel-217346?style=flat&logo=microsoft-excel&logoColor=white)

---
---

### PROJECTS

#### 🔹 [AI Resume Screening & Job Matching System](https://github.com/gurleenkaur0703/ai-resume-screening) | [Live Demo](https://ai-resume-screening-using.streamlit.app)
> **Tech Stack:** Python | PostgreSQL | Pandas | NLP | Streamlit
- **Pipeline & Ingestion:** Built an end-to-end data processing workflow to ingest, extract, and clean unstructured PDF/text resume data into structured relational records.
- **Database Architecture:** Executed complex SQL queries in PostgreSQL for data validation, filtering, and candidate-job matching algorithms.
- **Analytics & Interface:** Integrated NLP-driven scoring and built an interactive Streamlit application to surface candidate insights.

---

#### 🔹 [Seasonal Sales Dashboard](https://github.com/gurleenkaur0703/Exploratory-Data-Analysis) | [Live Demo](https://christmas-sales-eda.streamlit.app)
> **Tech Stack:** Python | Pandas | NumPy | Seaborn | Streamlit
- **ETL Workflows:** Built to ingest, and clean raw transactional sales data with null-handling and schema validation.
- **Data Modeling & EDA:** Conducted trend, anomaly, and seasonal pattern analysis to identify trends, anomalies, and patterns.
- **Dashboarding:** Built a interactive web application to visualize key performance metrics and operational revenue trends.

---

#### 🔹 [IPL Win Probability Predictor](https://github.com/gurleenkaur0703/ipl-win-probability-predictor) | [Live Demo](https://ipl-match-win-probability-predictor.streamlit.app)
> **Tech Stack:** Python | scikit-learn | Pandas | Streamlit
- **Feature Engineering:** Cleaned and transformed historical match telemetry datasets to build ML-ready feature matrices.
- **Predictive Inference:** Trained a Logistic Regression model to process historical match conditions and compute real-time win probabilities.

---

#### 🔹 [Music Store Analysis](https://github.com/gurleenkaur0703/Music_Store_Analysis)
> **Tech Stack:** MySQL | Python | Pandas | Seaborn
- **Relational Schema Design:** Designed normalized database models simulating music e-commerce and social platform backend interactions (Users, Posts, Likes, Transactions).
- **Advanced SQL Analytics:** Authored complex SQL queries (Aggregations, CTEs, Joins) to compute customer lifetime value, genre revenue shares, and user engagement metrics.
- **Data Export Pipeline:** Automated result set extraction into Pandas for visualization and spreadsheet reporting.

---
---

### ACHIEVEMENTS & PUBLICATIONS

- **Research Publication & Presentation:** Co-authored and presented research paper *"Precision Ranking: Harnessing NLP and Normalization for Enhanced Rating and Review Analysis"* at the 12th IEEE-sponsored ICETET SIP-25 Conference (Aug 2025).
- **Global Rank #89:** Secured Top 100 standing out of thousands of participants in the Women’s Day Coding Challenge 2025 hosted by Naukri Campus.
- **Naukri Campus Young Turks 2025:** Awarded Merit Certificate for top performance in nation-wide technical assessment.
- **Data Analytics Winner:** Selected as a top performer in the Analytics Vidhya Data Analyst Skill Assessment.
- **Academic Excellence:** Subject Topper in Informatics Practices (12th CBSE Board Examinations).

---
---

### CERTIFICATIONS

- **OOPs in C++** – Coding Ninjas (Oct 2025)
- **Python Essentials 1** – Cisco Networking Academy (Mar 2025)  
- **SQL (Intermediate)** – HackerRank (Dec 2024)  
- **Cloud Computing** – NPTEL (May 2024)  
- **RDBMS** – Coursera (Mar 2024) 

---
---

### PROFILES

- [![Gmail](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:gurleenkaur.gk03@gmail.com)
- [![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/gurleenkaur0703)
- [![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)](https://github.com/gurleenkaur0703)
- [![LeetCode](https://img.shields.io/badge/LeetCode-FFA116?style=flat&logo=leetcode&logoColor=black)](https://leetcode.com/u/gurleen_kaur26)
- [![GeeksforGeeks](https://img.shields.io/badge/GeeksforGeeks-0F9D58?style=flat&logo=geeksforgeeks&logoColor=white)](https://www.geeksforgeeks.org/user/gurleen_kaur26)
- [![Unstop](https://img.shields.io/badge/Unstop-1C4980?style=flat&logo=unstop&logoColor=white)](https://unstop.com/u/gurlekau48086)

---
---

### CURRENT FOCUS

- Building **end-to-end ETL pipelines and relational data warehouse architectures**
- Mastering **advanced SQL optimization, CTEs, and window functions**
- Expanding practical experience with **cloud data platforms and containerization**
- Preparing for **Data Engineering and Data Analytics** engineering roles

---
---

<!-- GitHub Activity Graph (High-Reliability Endpoint) 
<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=gurleenkaur0703&theme=react-dark&hide_border=true" width="100%" alt="Gurleen's GitHub Activity Graph" />
</p>
<p align="center">
  <img src="https://github-readme-stats-sigma-five.vercel.app/api?username=gurleenkaur0703&show_icons=true&theme=radical" height="160" alt="GitHub Stats" />
  <img src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=gurleenkaur0703&layout=compact&theme=radical" height="160" alt="Top Languages" />
</p>
-->

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=1000&color=2EF2B3&center=true&vCenter=true&width=650&lines=Thanks+for+visiting+my+profile!;Let's+connect+and+build+something+awesome!;Have+a+great+day!+%E2%9C%A8" alt="Thank You Animation" />
</p>
